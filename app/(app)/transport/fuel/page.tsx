...
import { listFuelLogs, listVehicles, listDrivers, deleteFuelLog } from "@/server/actions/transport";
import { listAccessibleCompanies } from "@/server/actions/company-list";
import { FuelDialog } from "./fuel-dialog";

export default async function FuelPage() {
  const [rows, vehiclesRaw, driversRaw, companies, activeId, crossCompany] = await Promise.all([
    listFuelLogs(),
    listVehicles(),
    listDrivers(),
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        action={<FuelDialog vehicles={vehicles} drivers={drivers} companies={companies} defaultCompanyId={activeId} />}
      />
      {rows.length === 0 ? (
        <EmptyState
            ...
            <TableBody>
              {rows.map(({ log, vehicle, driver, company }) => (
                <TableRow key={log.id}>
                  ...
                  <TableCell className="hidden md:table-cell text-sm">{log.station ?? "—"}</TableCell>
                  ...
                  <TableCell className="text-right">
                    <FuelDialog log={log} vehicles={vehicles} drivers={drivers} companies={companies} />
